...
// DOT : 데이터 이동 객체
    // - 복잡한 로직을 하는 곳이 아니다. 단순 데이터를 이동하기 위한
...
public class MemberDto {//class start
    ...
    public MemberDto(){}    // 기본생성자
        ...
        // 오른쪽클릭 -> 생성 -> 생성자 -> 선택한 필드 만큼의 생성자를 자동으로 만듬.
    ...
    // 3. 메소드
        // 1. setter , getter
    // 오른쪽 클릭 -> 생성 -> getter / setter -> 선택한 필드 만큼의 생성자를 자동으로 만듬.
        // 2. toString()        : 객체 필드정보 확인차 주로 사용 목적 ( 객체 호출시 주소값 대신 필드 호출하는 함수)
    // 오른쪽 클릭 -> 생성 -> toString -> 선택한 필드 만큼의 생성자를 자동으로 만듬.
    public int getNo() {
        return no;
    }
